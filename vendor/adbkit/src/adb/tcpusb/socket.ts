...
export default class Socket extends EventEmitter {
  ...
  public remoteId = new RollingCounter(UINT32_MAX);
  private services = new ServiceMap();
  private reverseManager?: ReverseForwardManager;
  ...
  constructor(
    ...
    if (this.options.reverseForwards) {
      this.reverseManager = new ReverseForwardManager(this, this.client, this.serial, this.options);
    }
  ...
  public end(): Socket {
    ...
    if (this.reverseManager) {
      this.reverseManager.end();
    }
  ...
  private _handleOpenPacket(packet: Packet): Bluebird<boolean | Service> {
    ...
    debug(`Calling ${nameStr}`);
    if (nameStr.startsWith('reverse:') && this.reverseManager) {
      return this._handleReverseOpenPacket(packet, nameStr);
    }
  ...
  private _handleReverseOpenPacket(packet: Packet, nameStr: string): Bluebird<boolean> {
    const remoteId = packet.arg0;
    const localId = this.remoteId.next();

    return Bluebird.try(() => {
      if (nameStr.startsWith('reverse:forward:')) {
        const spec = nameStr.substring('reverse:forward:'.length);
        const parts = spec.split(';');
        if (parts.length !== 2) {
          throw new Error(`Invalid reverse forward spec ${spec}`);
        }
        return this.reverseManager.handleForward(parts[0], parts[1]).then(() => null);
      } else if (nameStr.startsWith('reverse:killforward:')) {
        const devSpec = nameStr.substring('reverse:killforward:'.length);
        return this.reverseManager.handleKillForward(devSpec).then(() => null);
      } else if (nameStr === 'reverse:killforward-all') {
        return this.reverseManager.handleKillForwardAll().then(() => null);
      } else if (nameStr === 'reverse:list-forward') {
        return this.reverseManager.handleListForward();
      } else {
        throw new Error(`Unsupported reverse command ${nameStr}`);
      }
    })
      .then((output: Buffer | null) => {
        this.write(Packet.assemble(Packet.A_OKAY, localId, remoteId, null));
        if (output && output.length > 0) {
          this.write(Packet.assemble(Packet.A_WRTE, localId, remoteId, output));
        }
        this.write(Packet.assemble(Packet.A_CLSE, localId, remoteId, null));
        return true;
      })
      .catch((err: Error) => {
        debug(`Reverse command ${nameStr} failed: ${err.message}`);
        this.write(Packet.assemble(Packet.A_CLSE, 0, remoteId, null));
        return false;
      });
  }
  ...
  private _forwardServicePacket(packet: Packet): Promise<boolean | Service> {
    ...
    if (this.reverseManager) {
      const reverseStream = this.reverseManager.getStreamByLocalId(localId);
      if (reverseStream) {
        return reverseStream.handle(packet) as Promise<boolean>;
      }
    }
    debug('Received a packet to a service or reverse stream that may have been closed already');
