...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('tcpusb-reverse-forwards', {
      describe: 'Whether to enable adb reverse forwarding support in the tcpusb bridge.'
    , type: 'boolean'
    , default: false
    })
    .option('tcpusb-reverse-port-range', {
      describe: 'Port range (e.g. 7000-7100) for loopback reverse forward listeners.'
    , type: 'string'
    })
    .option('tcpusb-reverse-max-forwards', {
      describe: 'Maximum concurrent reverse forwards allowed per session.'
    , type: 'number'
    , default: 32
    })
    .option('tcpusb-reverse-max-streams', {
      describe: 'Maximum concurrent reverse streams allowed per session.'
    , type: 'number'
    , default: 64
    })
...
module.exports.handler = function(argv) {
  ...
  , fork: function(device, ports) {
      ...
      .concat(argv.tcpusbReverseForwards ? ['--tcpusb-reverse-forwards'] : [])
      .concat(argv.tcpusbReversePortRange ? ['--tcpusb-reverse-port-range', argv.tcpusbReversePortRange] : [])
      .concat(argv.tcpusbReverseMaxForwards ? ['--tcpusb-reverse-max-forwards', argv.tcpusbReverseMaxForwards] : [])
      .concat(argv.tcpusbReverseMaxStreams ? ['--tcpusb-reverse-max-streams', argv.tcpusbReverseMaxStreams] : [])
