...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, group, solo, urlformat) {
    ...
    plugin.start = function() {
      return new Promise(function(resolve, reject) {
        ...
        var server = adb.createTcpUsbBridge(options.serial, {
          reverseForwards: options.tcpusbReverseForwards
        , reversePortRange: options.tcpusbReversePortRange
        , reverseMaxForwards: options.tcpusbReverseMaxForwards
        , reverseMaxStreams: options.tcpusbReverseMaxStreams
        , auth: function(key) {
    ...
    plugin.stop = Promise.method(function() {
      ...
      return adb.client.getDevice(options.serial).transport()
        .then(function(transport) {
          var Protocol = require('@devicefarmer/adbkit/lib/adb/protocol')
          transport.write(Protocol.encodeData('reverse:killforward-all'))
          return transport.parser.readAscii(4).finally(function() {
            transport.end()
          })
        })
        .catch(function(err) {
          log.info('Reverse killforward-all cleanup note: %s', err.message)
        })
    })
